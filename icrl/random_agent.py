...
def cpg(config):
    use_cost_wrapper_train = False
    use_cost_wrapper_eval = False

    # Create the vectorized environments
    train_env = utils.make_eval_env(env_id=config.train_env_id,
    ...
    rets = utils.sample_from_agent(model, eval_env, 10)
    mean_reward = rets[-2]
    print("Reward", mean_reward)

    # For finding cost
    rets = utils.sample_from_agent(model, train_env, 10)
    obs, acs, lengths = rets[0], rets[2], rets[-1]
    costs = []
    start_idx = 0
    for i in range(len(lengths)):
        end_idx = start_idx + lengths[i]
        costs.append(np.mean(cost_function(obs[start_idx:end_idx, ...],
                                           acs[start_idx:end_idx, ...])))
        start_idx += lengths[i]
    print("Cost", costs)
